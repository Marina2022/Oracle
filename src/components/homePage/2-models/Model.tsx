...
const Model = ({model}: { model: ModelType }) => {
  ...
  return (
    <li>
      ...
      <Card className="gap-0 text-left flex flex-col h-full rounded-xl border glassmorphism p-6 hover:scale-105 transition-all duration-300 hover:neon-glow group cursor-pointer">
        ...
        <div className="space-y-3 mt-auto">
          ...
          <Badge className={hight ? "bg-chart-4/20 text-chart-4 w-full" : "bg-chart-5/20 text-chart-5 w-full"}>{hight ? "Высокая" : "Хорошая"} точность</Badge>
        </div>
      </Card>
    </li>
  );
